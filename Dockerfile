...
FROM nvidia/cuda${ARCH:+-$ARCH}:${CUDA}-cudnn${CUDNN}-runtime-ubuntu${UBUNTU_VERSION} as python-builder
...
#RUN mv /etc/apt/sources.list.d/cuda.list /etc/apt/sources.list.d/cuda.list.old && \
#    mv /etc/apt/sources.list.d/nvidia-ml.list /etc/apt/sources.list.d/nvidia-ml.list.old
